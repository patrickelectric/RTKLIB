...
static int decode_sbsfast(raw_t *raw)
{
    ...
    if(raw->nav.sbssat.iodp!=U1(p+10)) {
        return 0;
    }
    if(type>5) {
        return -1;
...
static int decode_sbsintegriy(raw_t *raw)
{
    ...
    for(i=0; i<raw->nav.sbssat.nsat&&i<MAXSAT; i++) {
        if(raw->nav.sbssat.sat[i].fcorr.iodf!=iodf[i/13]) {
            continue;
        }
        udre=U1(p+14+i);
...
static int decode_sbsionodelay(raw_t *raw)
{
    ...
    if(band>MAXBAND||raw->nav.sbsion[band].iodi!=U1(p+10)) {
        return 0;
    }
...
#if 0 /* unused */
    case ID_GALRAWFNAV:
        return decode_galrawfnav(raw); /* not yet supported in RTKLIB */
    case ID_GLOALM:
        return decode_glosalm(raw); /* not yet supported in RTKLIB */

    case ID_PVTGEOD:
        return decode_pvtgeod(raw);
    case ID_RXSETUP:
        return decode_rxsetup(raw);
    case ID_COMMENT:
        return decode_comment(raw);
#endif
    default:
