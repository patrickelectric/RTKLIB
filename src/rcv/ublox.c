...
static void setI1(unsigned char *p, char           i)
{
    *p=(unsigned char)i;
}
...
static int decode_rxmrawx(raw_t *raw)
{
    ...
    for(i=0,p+=16; i<nsat&&i<MAXOBS; i++,p+=32) {
        ...
        if(lockt==0||lockt<raw->lockt[sat-1][0]) {
            raw->lockt[sat-1][1]=1;
        }
        raw->lockt[sat-1][0]=lockt;
        if(std_slip>0) {
            if(cpstd>=std_slip) {
                raw->lockt[sat-1][1]=1;    /* slip by std-dev of cp */
            }
        }
...
static int decode_trkmeas(raw_t *raw)
{
    ...
    for(i=0,p=raw->buff+110; i<nch; i++,p+=56) {
        ...
        if(sys==SYS_GLO&&frq>=-7&&frq<=7) {
            if(fw==2) {
                raw->obs.data[n].P[0]+=(double)P_adj_fw2[frq+7];
            }
            if(fw==3) {
                raw->obs.data[n].P[0]+=(double)P_adj_fw3[frq+7];
            }
        }
