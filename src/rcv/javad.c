...
static double freq_glo(int freq, int freqn)
{
    switch(freq) {
    case 0:
        return FREQ1_GLO+DFRQ1_GLO*freqn;
    case 1:
        return FREQ2_GLO+DFRQ2_GLO*freqn;
    }
    return 0.0;
}
/* checksum ------------------------------------------------------------------*/
...
static int decode_javad(raw_t *raw)
{
    ...
    if(!strncmp(p,"QA",2)) {
        return decode_QA(raw);    /* qzss almanac (ext) */
    }
    ...
    if(!strncmp(p,"WE",2)) {
        return decode_WE(raw);    /* waas ephemeris */
    }
    if(!strncmp(p,"QE",2)) {
        return decode_QE(raw);    /* qzss ephemeris (ext) */
    }
    if(!strncmp(p,"CN",2)) {
        return decode_CN(raw);    /* beidou ephemeris (ext) */
    }
    ...
    if(!strncmp(p,"ED",2)) {
        return decode_nd(raw,SYS_GAL);    /* raw navigation data */
    }
    if(!strncmp(p,"cd",2)) {
        return decode_nd(raw,SYS_CMP);    /* raw navigation data */
    }
    if(!strncmp(p,"LD",2)) {
