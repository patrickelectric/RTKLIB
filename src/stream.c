...
static int accsock(tcpsvr_t *tcpsvr, char *msg)
{
    ...
    for(i=0; i<MAXCLI; i++) if(tcpsvr->cli[i].state==0) {
            break;
        }
    if(i>=MAXCLI) {
...
extern int strread(stream_t *stream, unsigned char *buff, int n)
{
    ...
    if(!(stream->mode&STR_MODE_R)||stream->state==0) {
        return 0;
    }
    if(stream->state == -1) {
...
extern int strwrite(stream_t *stream, unsigned char *buff, int n)
{
    ...
    if(!(stream->mode&STR_MODE_W) || stream->state == 0) {
        return 0;
    }
    if(stream->state == -1) {
        /* Try to open serial port */
