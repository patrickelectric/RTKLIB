                        ...
                        char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
    ...
    if(strstr(label,"MARKER NAME")) {
        ...
        *tobs[i][nt]='\0';
        ...
        if(i==5) {
            for(j=0; j<nt; j++) if(tobs[i][j][1]=='2') {
                    tobs[i][j][1]='1';
                }
        }
...
extern int init_rnxctr(rnxctr_t *rnx)
{
    ...
    rnx->ver=0.0;
    rnx->sys=rnx->tsys=0;
    for(i=0; i<6; i++) for(j=0; j<MAXOBSTYPE; j++) {
            rnx->tobs[i][j][0]='\0';
        }
    rnx->obs.n=0;
    rnx->nav.n=MAXSAT;
...
extern int outrnxobsh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    for(i=0; i<3; i++) if(fabs(opt->apppos[i])<1E8) {
            pos[i]=opt->apppos[i];
        }
    for(i=0; i<3; i++) if(fabs(opt->antdel[i])<1E8) {
            del[i]=opt->antdel[i];
        }
                      ...
                      int flag)
{
    ...
    for(i=0; i<ns; i++) {
        ...
        for(j=0; j<opt->nobs[m]; j++) {
            ...
            case 'C':
            case 'P':
                outrnxobsf(fp,obs[ind[i]].P[k],-1,obs[ind[i]].qualP[k]);
                break;
            case 'L':
                outrnxobsf(fp,obs[ind[i]].L[k],obs[ind[i]].LLI[k],obs[ind[i]].qualL[k]);
                break;
            case 'D':
                outrnxobsf(fp,obs[ind[i]].D[k],-1,-1);
                break;
            case 'S':
                outrnxobsf(fp,obs[ind[i]].SNR[k]*0.25,-1,-1);
                break;
            }
        }
