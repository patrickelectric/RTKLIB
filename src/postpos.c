                      ...
                      const solopt_t *sopt)
{
    ...
    if(sopt->outhead) {
        ...
        for(i=0; i<obss.n; i++)    if(obss.data[i].rcv==1) {
                break;
            }
        for(j=obss.n-1; j>=0; j--) if(obss.data[j].rcv==1) {
                break;
            }
        if(j<i) {
            fprintf(fp,"\n%s no rover obs data\n",COMMENTH);
...
static int nextobsf(const obs_t *obs, int *i, int rcv)
{
    ...
    for(; *i<obs->n; (*i)++) if(obs->data[*i].rcv==rcv) {
            break;
        }
    for(n=0; *i+n<obs->n; n++) {
...
static int nextobsb(const obs_t *obs, int *i, int rcv)
{
    ...
    for(; *i>=0; (*i)--) if(obs->data[*i].rcv==rcv) {
            break;
        }
    for(n=0; *i-n>=0; n++) {
        tt=timediff(obs->data[*i-n].time,obs->data[*i].time);
                      ...
                      obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    if(ts.time==0||te.time==0) {
        ...
        for(j=obs->n-1; j>=0; j--) if(obs->data[j].rcv==1) {
                break;
            }
        if(i<j) {
            if(ts.time==0) {
                     ...
                     const char *rov)
{
    ...
    for(i=0; i<n; i++) if(strstr(infile[i],"%r")) {
            break;
        }
                     ...
                     const char *rov, const char *base)
{
    ...
    for(i=0; i<n; i++) if(strstr(infile[i],"%b")) {
            break;
        }
