                       ...
                       const nav_t *nav)
{
    ...
    for(f=0; f<rtk->opt.nf; f++) {
        ...
        if(fabs(tt=timediff(obs[i].time,rtk->pt[rcv-1][sat-1][f]))<DTTOL) {
            continue;
        }
    ...
    for(m=0; m<4; m++) {
        ...
        for(f=opt->mode>PMODE_DGPS?0:nf; f<nf*2; f++) {
            ...
            for(j=0; j<ns; j++) {
                ...
                /* save residuals */
                if(f<nf) {
                    rtk->ssat[sat[j]-1].resc[f   ]=v[nv];    /* carrier phase innovations */
                } else {
                    rtk->ssat[sat[j]-1].resp[f-nf]=v[nv];    /* pseudorange measurements */
                }
                      ...
                      rtk_t *rtk, double *y)
{
    ...
    for(i=0; i<n; i++) {
        for(j=0; j<nb; j++) if(obsb[j].sat==obs[i].sat) {
                break;
            }
        if(j>=nb) {
            continue;
...
static int ddmat(rtk_t *rtk, double *D,int gps,int glo,int sbs)
{
    ...
    if(nb>0) {
        trace(3,"refSats=");
        tracemat(3,ref,1,nb,2,0);
        trace(3,"fixSats=");
        tracemat(3,fix,1,nb,2,0);
    }
    return nb;
}
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa,int gps,int glo,int sbs)
{
    ...
    if(!(info=lambda(nb,2,y+na,Qb,b,s))) {
        ...
        trace(3,"N(1)=");
        tracemat(3,b,1,nb,10,3);
        trace(3,"N(2)=");
        tracemat(3,b+nb,1,nb,10,3);
                  ...
                  const nav_t *nav)
{
    ...
    for(i=j=0; i<MAXSAT; i++) {
        if(rtk->ssat[i].azel[1] <= 0) {
            continue;
        }
        rtk->sol.azim[j] = rtk->ssat[i].azel[0] * R2D;
        if(rtk->sol.azim[j] < 0.0) {
            rtk->sol.azim[j] += 360.0;
        }
        ...
        case SYS_GPS:
            rtk->sol.typeSV[j] = 0;
            rtk->sol.idSV[j] = prn-MINPRNGPS+1;
            break;
        case SYS_GLO:
            rtk->sol.typeSV[j] = 1;
            rtk->sol.idSV[j] = prn-MINPRNGLO+1;
            break;
        case SYS_GAL:
            rtk->sol.typeSV[j] = 2;
            rtk->sol.idSV[j] = prn-MINPRNGAL+1;
            break;
        case SYS_QZS:
            rtk->sol.typeSV[j] = 3;
            rtk->sol.idSV[j] = prn-MINPRNQZS+1;
            break;
        case SYS_CMP:
            rtk->sol.typeSV[j] = 4;
            rtk->sol.idSV[j] = prn-MINPRNCMP+1;
            break;
        case SYS_LEO:
            rtk->sol.typeSV[j] = 5;
            rtk->sol.idSV[j] = prn-MINPRNLEO+1;
            break;
        case SYS_SBS:
            rtk->sol.typeSV[j] = 6;
            rtk->sol.idSV[j] = prn;
    ...
    for(i=0; i<MAXSAT; i++) {
        ...
        for(j=0; j<NFREQ; j++) {
            rtk->ssat[i].vsat[j]=rtk->ssat[i].snr[j]=0;    /* valid satellite */
        }
    }
