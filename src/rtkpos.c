                 ...
                 double *H, double *R, int *vflg)
{
    ...
    for (m=0;m<4;m++) { 
        ...
        for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
            ...
            /* double-differenced residual measurements from 2 receivers and 2 sats in meters */
                  ...
                  const nav_t *nav)
{
    ...
    float ratio1;
    int i,j,f,n=nu+nr,ns,ny,nv,sat[MAXSAT],iu[MAXSAT],ir[MAXSAT],niter,prn,dly;
...
>>>>>>> Brought back the DOP functionality
    for (i=0;i<MAXSAT;i++) {
        rtk->ssat[i].sys=satsys(i+1,NULL);
        for (j=0;j<NFREQ;j++) rtk->ssat[i].vsat[j]=0; /* gps system */
        for (j=1;j<NFREQ;j++) rtk->ssat[i].snr [j]=0; /* valid satellite */
    }
