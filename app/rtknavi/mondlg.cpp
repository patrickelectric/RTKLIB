...
void __fastcall TMonitorDialog::ShowObs(void)
{
    ...
    for(i=0; i<n; i++) {
        ...
        for(k=0; k<NFREQ+nex; k++) {
            ...
            if(*code) {
                Tbl->Cells[j++][i+1]=s.sprintf("L%s",code);
            } else {
                Tbl->Cells[j++][i+1]="";
            }
        }
...
void __fastcall TMonitorDialog::ShowStr(void)
{
    ...
    for(i=0; i<9; i++) {
        ...
        pp=path;
        if((p=strchr(path,'@'))) {
            for(q=p-1; q>=path; q--) if(*q==':') {
                    break;
                }
            if(q>=path) for(q++; q<p; q++) {
                    *q='*';
                }
        }
