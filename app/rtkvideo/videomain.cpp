...
void __fastcall TMainForm::LoadOptions(void)
{
    ...
    while(fgets(buff, sizeof(buff), fp)) {
        if(*buff == '[') {
            ...
            if(sscanf(buff, "captionpos = %d",   &val)) {
                CaptionPos  = val;
            } else if(sscanf(buff, "captionsize = %d",  &val)) {
                CaptionSize = val;
            } else if(sscanf(buff, "captioncolor = %d", &val)) {
                CaptionColor = val;
            } else if(sscanf(buff, "tcpportena = %d",   &val)) {
                TcpPortEna  = val;
            } else if(sscanf(buff, "tcpportno = %d",    &val)) {
                TcpPortNo   = val;
            } else if(sscanf(buff, "outfileena = %d",   &val)) {
                OutFileEna  = val;
            } else if(sscanf(buff, "outfile = %1023[^\r\n]", str)) {
                OutFile = str;
            } else if(sscanf(buff, "outtimetag = %d",   &val)) {
                OutTimeTag  = val;
            } else if(sscanf(buff, "fileswap = %d",	   &val)) {
                FileSwap    = val;
