...
void Plot::DrawTrkVel(QPainter &c,const TIMEPOS *vel)
{
    ...
    p1.ry()-=SIZE_VELC/2;
    if(v>=1.0) {
        DrawMark(GraphT,c,p1,10,CColor[2],SIZE_VELC,90-static_cast<int>(dir));
    }
...
void Plot::DrawSol(QPainter &c,int level, int type)
{
    ...
    j=-1;
    for(i=0; i<3; i++) if(btn[i]->isChecked()) {
            j=i;
        }
    for(i=0; i<3; i++) {
        if(!btn[i]->isChecked()) {
...
void Plot::DrawObsEphem(QPainter &c,double *yp)
{
    ...
    for(i=0; i<MAXSAT; i++) {
        ...
        for(j=0; j<Nav.n; j++) {
            ...
            if(in) {
                GraphR->DrawMark(c,ps[2],0,svh?MColor[0][5]:CColor[1],svh?4:3,0);
            }
        }
        for(j=0; j<Nav.ng; j++) {
...
void Plot::DrawSnrE(QPainter &c,int level)
{
    ...
    j=0;
    for(i=0; i<2; i++) if(btn[i]->isChecked()) {
            j=i;
        }
    for(i=0; i<2; i++) {
        if(!btn[i]->isChecked()) {
            ...
            for(int i=0; i<3; i++) {
                ...
                if(level&&PlotStyle<2) {
                    ...
                    for(int j=0; j<m; j++) {
                        ...
                        if(i==2) {
                            GraphG[i]->DrawMark(c,x[j],y[3][j],0,MColor[0][4],MarkSize,0);
                        }
                    }
                }
                if(level&&i==1) {  /* slip */
