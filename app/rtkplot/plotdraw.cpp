...
void __fastcall TPlot::DrawObsEphem(double *yp)
{
    ...
    for(i=0; i<MAXSAT; i++) {
        ...
        for(j=0; j<Nav.n; j++) {
            ...
            if(in) {
                GraphR->DrawMark(ps[2],0,svh?MColor[0][5]:CColor[1],svh?4:3,0);
            }
        }
        for(j=0; j<Nav.ng; j++) {
    ...
    if(ShowStats&&BtnShowTrack->Down&&0<=ind&&ind<NObs&&!SimObs) {
        ...
        for(i=IndexObs[ind]; i<Obs.n&&i<IndexObs[ind+1]; i++) {
            ...
            if(!*code) {
                ...
                s+=" : ";
                for(j=0; j<NFREQ; j++) {
                    s+=ss.sprintf("%02.0f ",obs->SNR[j]*0.25);
                }
                s+=": ";
                for(j=0; j<NFREQ; j++) {
                    s+=ss.sprintf("%d",obs->LLI[j]);
                }
            } else {
                for(j=0; j<NFREQ+NEXOBS; j++) {
    ...
    if(NObs>0&&BtnSol1->Down) {
        ...
        for(i=l=0; i<3; i++) {
            ...
            for(sat=1,np=0; sat<=MAXSAT; sat++) {
                ...
                if(!level||!(PlotStyle%2)) {
                    for(j=0; j<n; j=k) {
                        for(k=j+1; k<n; k++) if(fabs(y[k-1]-y[k])>30.0) {
                                break;
                            }
        ...
        for(int sat=1; sat<=MAXSAT; sat++) {
            ...
            for(int i=0; i<3; i++) {
                ...
                if(!level||!(PlotStyle%2)) {
                    ...
                    if(i==2) {
                        DrawPolyS(GraphG[i],x,y[3],m,CColor[3],0);
                    }
                }
                if(level&&PlotStyle<2) {
                    TColor color;
                    for(int j=0; j<m; j++) {
                        ...
                        if(i==2) {
                            GraphG[i]->DrawMark(x[j],y[3][j],0,MColor[0][4],MarkSize,0);
                        }
                    }
                }
                if(level&&i==1) {  /* slip */
                                 ...
                                 TColor color, int style)
{
    ...
    for(i=0; i<n; i=j) {
        for(j=i+1; j<n; j++) if(fabs(x[j]-x[j-1])>TBRK) {
                break;
            }
