...
__fastcall TPlot::TPlot(TComponent* Owner) : TForm(Owner)
{
    ...
    if(nfreq>=2) {
        FrqType->Items->Add("L2");
    }
    if(nfreq>=3) {
        FrqType->Items->Add("L5");
    }
    if(nfreq>=4) {
        FrqType->Items->Add("L6");
    }
    if(nfreq>=5) {
        FrqType->Items->Add("L7");
    }
    if(nfreq>=6) {
        FrqType->Items->Add("L8");
    }
    FrqType->ItemIndex=0;
...
void __fastcall TPlot::MenuMapLayerClick(TObject *Sender)
{
    ...
    if(VecMapDialog->ShowModal()!=mrOk) {
        return;
    }
...
void __fastcall TPlot::MenuQcObsClick(TObject *Sender)
{
    ...
    for(i=0; i<ObsFiles->Count; i++) {
        cmd+=" \""+ObsFiles->Strings[i]+"\"";
    }
    for(i=0; i<NavFiles->Count; i++) {
        cmd+=" \""+NavFiles->Strings[i]+"\"";
    }
